...
if (isset($_GET['kode_provinsi'])) {
    ...
    $query = "SELECT * FROM oneway WHERE kode_provinsi = ?";
...
// Fetch all kode_provinsi and kode_kota from the database
...
if (isset($_POST['submit'])) {
    ...
    $kode_kota_new = $_POST['kode_kota'];
    $besaran_new = $_POST['besaran'];

    // Update the data in the database
    $query = "UPDATE oneway SET besaran = ?, kode_kota = ? WHERE kode_provinsi = ?";
    // Periksa apakah persiapan pernyataan SQL berhasil
    if ($stmt = $koneksi->prepare($query)) {
        $stmt->bind_param("sss", $besaran_new, $kode_kota_new, $kode_provinsi_new);

        if ($stmt->execute()) {
            // If update successful, display a success message and redirect to the data_oneway.php page
...
<body>
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 mt-14">
            <h1 class="text-[#23212F] font-medium text-2xl">Edit Data Transportasi Oneway</h1>
            <div class="border-solid">
                <div class="bg-[#193D5E] rounded-t-xl mt-4 p-4">
                    <h1 class="text-white text-md font-semibold">Edit Data Transportasi Oneway</h1>
                </div>
                <div class="shadow-lg px-5 py-3">
                    <form action="" method="POST">
                        <div class="mb-6 flex">
                            ...
                            <div class="md:w-5/6">
                                <select id="kode_provinsi" name="kode_provinsi" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2" required>
                                    <option value="" disabled>Pilih Provinsi</option>
                                    <?php
                                    // Loop through each provinsi data
                                    while ($provinsi_row = $provinsi_result->fetch_assoc()) :
                                        $kode_provinsi_option = $provinsi_row['kode_provinsi'];
                                        $ibukota_provinsi = $provinsi_row['ibukota_provinsi'];
                                    ?>
                                        <option value="<?php echo $kode_provinsi_option; ?>" <?php if ($kode_provinsi == $kode_provinsi_option) echo 'selected'; ?>><?php echo $ibukota_provinsi; ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                        </div>
                        <div class="mb-6 flex">
                            <div class="md:w-2/6">
                                <label for="kode_kota" class="block text-sm font-medium text-[#102E48] mt-2">Kota</label>
                            </div>
                            <div class="md:w-5/6">
                                <select id="kode_kota" name="kode_kota" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2" required>
                                    <option value="" disabled>Pilih Kota</option>
                                    <?php
                                    // Loop through each kota data
                                    while ($kota_row = $kota_result->fetch_assoc()) :
                                        $kode_kota_option = $kota_row['kode_kota'];
                                        $nama_kota = $kota_row['nama_kota'];
                                    ?>
                                        <option value="<?php echo $kode_kota_option; ?>" <?php if ($kode_kota == $kode_kota_option) echo 'selected'; ?>><?php echo $nama_kota; ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <a href="data_oneway.php" class="text-blue-500 hover:text-blue-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Back</a>
                        </div>
                    </form>
                </div>
            </div>
