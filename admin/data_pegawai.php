...
<body>
    <section id="sidebar">
                        ...
                        <ul id="dropdown-mb" class="hidden py-2 space-y-2 text-sm">
                            <li>
                                <button type="button" class="flex items-center p-2 pr-6 rounded-lg hover:bg-[#D6F170] text-gray-500 hover:text-black group" aria-controls="dropdown-uha" data-collapse-toggle="dropdown-uha">
                                    <span class="flex-1 ms-3 whitespace-nowrap text-sm pr-12">Uang Harian</span>
        ...
        <div class="p-4 sm:ml-64">
            <div class="p-4 mt-14">
                <h1 class="text-[#23212F] font-medium text-2xl">Data Pegawai</h1>
                    ...
                    <div class="shadow-lg px-5 py-3">
                                    ...
                                    <div class="relative p-4 w-full max-w-2xl max-h-full">
                                        ...
                                        <div class="relative bg-[#F3F3F3] rounded-2xl shadow">
                                            ...
                                            <div class="p-4 md:p-5 space-y-4">
                                                <form class="space-y-4" action="proses_insert_dtpg.php" method="POST">
                                                    <div class="mb-6 flex">
                                                        <div class="md:w-2/6">
                                                            <label for="nip" class="block text-sm font-medium text-[#102E48] mt-2">NIP Pegawai</label>
                                                        </div>
                                                        <div class="md:w-5/6">
                                                            <input type="text" id="nip" name="nip" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2 placeholder-gray-300" placeholder="Masukkan NIP pegawai..." required />
                                                        </div>
                                                    </div>
                                                    <div class="mb-6 flex">
                                                        <div class="md:w-2/6">
                                                        ...
                                                        <div class="md:w-5/6">
                                                            <input type="text" id="nama" name="nama" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2 placeholder-gray-300" placeholder="Masukkan nama pegawai..." required />
                                                        </div>
                                                    </div>
                                                    <div class="mb-6 flex">
                                                        <div class="md:w-2/6">
                                                            <label for="golongan" class="block text-sm font-medium text-[#102E48] mt-2">Pangkat/Gol Pegawai</label>
                                                        </div>
                                                        <div class="md:w-5/6">
                                                            <select id="golongan" name="golongan" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2 placeholder-gray-300" required>
                                                                ...
                                                                if ($result->num_rows > 0) {
                                                                    while ($row = $result->fetch_assoc()) {
                                                                        echo "<option value='" . $row['kode_golongan'] . "'>" . $row['jenis_golongan'] . "</option>";
                                                                    }
                                                                }
                                                                ?>
                                                            </select>
                                                    ...
                                                    <div class="mb-6 flex">
                                                        ...
                                                        <div class="md:w-5/6">
                                                            <select id="jabatan" name="jabatan" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2 placeholder-gray-300" required>
                                                                ...
                                                                if ($result->num_rows > 0) {
                                                                    while ($row = $result->fetch_assoc()) {
                                                                        echo "<option value='" . $row['kode_jabatan'] . "'>" . $row['jenis_jabatan'] . "</option>";
                                                                    }
                                                                }
                                                                ?>
                                                            </select>
                                                    ...
                                                    <div class="flex justify-end">
                                                        ...
                                                        <button type="button" class="text-white bg-slate-500 hover:bg-slate-600 font-medium rounded-lg text-xs px-5 py-2 me-2 mb-2" data-modal-hide="default-modal">Kembali</button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
                        ...
                        <div class="overflow-x-auto shadow-lg sm:rounded-lg mt-3">
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                                ...
                                <tbody>
                                    <?php
                                    // Prepare SQL query to fetch data from database with LIMIT based on selected entries per page
                                    $query = "SELECT pegawai.nip, pegawai.nama, jabatan.jenis_jabatan, golongan.jenis_golongan
                                            FROM pegawai
                                            JOIN jabatan ON pegawai.kode_jabatan = jabatan.kode_jabatan
                                            JOIN golongan ON pegawai.kode_golongan = golongan.kode_golongan
                                    ...
                                    if ($result) {
                                        if ($result->num_rows > 0) {
                                            ...
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                echo "<td class='px-3 py-3 text-center'>" . $nomor++ . "</td>";
                                                echo "<td class='px-3 py-3 text-center'>" . $row['nip'] . "</td>"; // Menggunakan 'nip' sebagai ID pegawai
                                                echo "<td class='px-3 py-3 text-center'>" . $row['nama'] . "</td>"; // Menggunakan 'nama' sebagai nama pegawai
                                                echo "<td class='px-3 py-3 text-center'>" . $row['jenis_jabatan'] . "</td>"; // Menggunakan 'kode_golongan' sebagai golongan pegawai
                                                echo "<td class='px-3 py-3 text-center'>" . $row['jenis_golongan'] . "</td>"; // Menggunakan 'kode_jabatan' sebagai jabatan pegawai
                                                echo "<td class='px-3 py-3 text-center'>
                                                    <a href='edit_dtpg.php?edit=" . $row['nip'] . "' class='font-medium bg-amber-400 hover:bg-amber-500 text-black p-2 rounded-md text-xs mr-2 text-center'>
                                                        <i class='fa-solid fa-file-circle-check'></i>
                                                    </a> 
                                                    <a href='proses_delete_dtpg.php?delete=" . $row['nip'] . "' onclick='return confirmDelete()' type='button' class='font-medium bg-[#ff0000] hover:bg-red-800 text-[#ffdead] p-2 rounded-md text-xs text-center'>
                                    ...
                                    <script>
                                        function confirmDelete() {
                                            return confirm('Apakah Kamu Yakin Untuk Menghapus Data Ini??');
                                        }
                                    </script>
