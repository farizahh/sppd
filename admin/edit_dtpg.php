...
if (isset($_GET['edit'])) {
    ...
    echo "NIP not provided!";
    exit();
}

// Fetch all jabatan from the database
$jabatan_query = "SELECT * FROM jabatan";
$jabatan_result = $koneksi->query($jabatan_query);

// Fetch all golongan from the database
$golongan_query = "SELECT * FROM golongan";
$golongan_result = $koneksi->query($golongan_query);
...
if (isset($_POST['submit'])) {
    ...
    $kode_golongan_new = $_POST['kode_golongan'];

    // Update the data in the database
    $query = "UPDATE pegawai SET nip = ?, nama = ?, kode_jabatan = ?, kode_golongan = ? WHERE nip = ?";
    $stmt = $koneksi->prepare($query);
    $stmt->bind_param("sssss", $nip_new, $nama_new, $kode_jabatan_new, $kode_golongan_new, $nip);
...
<body>
    <section id="sidebar">
        ...
        <div class="p-4 sm:ml-64">
            <div class="p-4 mt-14">
                <h1 class="text-[#23212F] font-medium text-2xl">Edit Data Pegawai</h1>
                <div class="border-solid">
                    <div class="bg-[#193D5E] rounded-t-xl mt-4 p-4">
                        <h1 class="text-white text-md font-semibold">Edit Data Pegawai</h1>
                    </div>
                    <div class="shadow-lg px-5 py-3">
                        <form action="" method="POST">
                            <div class="mb-6 flex">
                                ...
                                <div class="md:w-5/6">
                                    <input type="text" id="nip" name="nip" value="<?php echo $nip; ?>" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2 placeholder-gray-300" placeholder="Masukkan NIP pegawai..." required />
                                </div>
                            </div>
                            <div class="mb-6 flex">
                                <div class="md:w-2/6">
                                ...
                                <div class="md:w-5/6">
                                    <input type="text" id="nama" name="nama" value="<?php echo $nama; ?>" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2 placeholder-gray-300" placeholder="Masukkan nama pegawai..." required />
                                </div>
                            </div>
                            <div class="mb-6 flex">
                                <div class="md:w-2/6">
                                    <label for="kode_jabatan" class="block text-sm font-medium text-[#102E48] mt-2">Jabatan Pegawai</label>
                                </div>
                                <div class="md:w-5/6">
                                    <select id="kode_jabatan" name="kode_jabatan" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2">
                                        ...
                                        <?php while ($jabatan_row = $jabatan_result->fetch_assoc()) : ?>
                            ...
                            <div class="mb-6 flex">
                                <div class="md:w-2/6">
                                    <label for="kode_golongan" class="block text-sm font-medium text-[#102E48] mt-2">Golongan Pegawai</label>
                                </div>
                                <div class="md:w-5/6">
                                    <select id="kode_golongan" name="kode_golongan" class="bg-white border border-gray-400 text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 w-full p-2">
                                        <option value="" disabled>Pilih Golongan</option>
                                        <?php while ($golongan_row = $golongan_result->fetch_assoc()) : ?>
                                            <option value="<?php echo $golongan_row['kode_golongan']; ?>" <?php if ($kode_golongan == $golongan_row['kode_golongan']) echo 'selected'; ?>><?php echo $golongan_row['jenis_golongan']; ?></option>
                                        <?php endwhile; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="flex items-center">
                                ...
                                <a href="data_pegawai.php" class="text-blue-500 hover:text-blue-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Back</a>
                            </div>
                        </form>
                    </div>
                </div>
