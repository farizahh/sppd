...
if (isset($_GET['edit'])) {
    ...
    $query = "SELECT * FROM provinsi WHERE kode_provinsi = ?";
    ...
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            $ibukota_provinsi = $row['ibukota_provinsi']; // Ensure this field is fetched
        } else {
            echo "Data not found!";
            exit();
        }
...
if (isset($_POST['submit'])) {
    ...
    $nama_provinsi_new = $_POST['nama_provinsi'];
    $ibukota_provinsi_new = $_POST['ibukota_provinsi']; // Retrieve the new capital city

    // Update the data in the database
    $query = "UPDATE provinsi SET kode_provinsi = ?, nama_provinsi = ?, ibukota_provinsi = ? WHERE kode_provinsi = ?";
    $stmt = $koneksi->prepare($query);

    // Check if the statement was prepared successfully
    if ($stmt) {
        $stmt->bind_param("ssss", $kode_provinsi_new, $nama_provinsi_new, $ibukota_provinsi_new, $kode_provinsi);
        if ($stmt->execute()) {
...
<body>
    <section id="sidebar">
        ...
        <div class="p-4 sm:ml-64">
            <div class="p-4 mt-14">
                <h1 class="bg-gradient-to-r from-sky-900 to-sky-700 text-white font-medium text-md px-5 py-3 rounded-xl">Data Provinsi</h1>
                <form action="" method="post">
                    <div class="mt-5 mb-6 flex">
                        <div class="md:w-1/6">
                            <label for="kode_provinsi" class="block text-sm font-medium text-[#102E48] mt-2">Kode Provinsi </label>
                        </div>
                        <div class="md:w-5/6">
                            <input type="text" id="kode_provinsi" name="kode_provinsi" value="<?php echo htmlspecialchars($kode_provinsi); ?>" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2" required />
                        </div>
                    </div>
                    <div class="mt-3 mb-6 flex">
                        <div class="md:w-1/6">
                            <label for="nama_provinsi" class="block text-sm font-medium text-[#102E48] mt-2">Nama Provinsi </label>
                        </div>
                        <div class="md:w-5/6">
                            <input type="text" id="nama_provinsi" name="nama_provinsi" value="<?php echo htmlspecialchars($nama_provinsi); ?>" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2" required />
                        </div>
                    </div>
                    <div class="mt-3 mb-6 flex">
                        <div class="md:w-1/6">
                            <label for="ibukota_provinsi" class="block text-sm font-medium text-[#102E48] mt-2">Ibukota Provinsi </label>
                        </div>
                        <div class="md:w-5/6">
                            <input type="text" id="ibukota_provinsi" name="ibukota_provinsi" value="<?php echo htmlspecialchars($ibukota_provinsi); ?>" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2" required />
                        </div>
                    </div>
                    <div class="flex float-right mb-10">
                        <button type="button" onclick="window.location.href = 'data_provinsi.php';" class="text-white bg-slate-500 hover:bg-slate-600 font-medium rounded-lg text-xs px-5 py-2 me-2 mb-2">Kembali</button>
